...
Make sure you install all dependencies first, run npm -i or run yarn (shorthand for npm install)
...
An output is minified via --mode flag which is set to production (already minified, optimized bundle) - additionally **/dist** directory will be removed every time when you run this script to make sure there are no artifacts left in the output location directory.
